    ...
    >
      <div className="flex flex-col md:flex-row items-center justify-center gap-2 md:gap-3 bg-black/40 backdrop-blur-xl px-5 py-3 md:px-5 md:py-2.5 rounded-3xl md:rounded-full border border-white/10 shadow-[0_0_30px_rgba(34,197,94,0.1)] w-full max-w-[280px] md:max-w-none">
        ...
        <div className="flex flex-row items-center justify-center gap-1.5 md:gap-1 font-mono tracking-wider text-xs md:text-sm font-bold w-full md:w-auto">
          <span className="bg-white/5 border border-white/10 px-2 md:px-2 py-1 rounded text-white flex-1 md:flex-none text-center">
          ...
          <span className="text-white/30 px-0.5">:</span>
          <span className="bg-nomad-green/10 border border-nomad-green/30 px-2 md:px-2 py-1 rounded text-nomad-green flex-1 md:flex-none text-center">
...
export function NavPanelContent({
  ...
  return (
    <>
      <div className="h-20 md:h-28 p-4 md:p-8 flex justify-start items-center border-b border-white/10 shrink-0">
        <div className="flex items-center gap-3">
          <img
            src="https://www.dropbox.com/scl/fi/eez8in6tuf5mgf3b4scz1/Nomad.svg?rlkey=6x9d65a0tljcelq7n6gmiy9px&st=u039rw9p&raw=1"
            alt="Nomad Logo"
            className="h-6 filter brightness-0 invert opacity-90"
            style={{
              filter:
                "brightness(0) saturate(100%) invert(67%) sepia(50%) saturate(579%) hue-rotate(63deg) brightness(98%) contrast(93%)",
            }}
          />
          <span className="text-2xl font-display font-black tracking-tighter text-nomad-ivory uppercase">
            Nomad
          </span>
        </div>
      </div>
      ...
      {posts.length > 0 && (
        <div className="border-b border-white/10 p-3 md:p-4 shrink-0 transition-opacity bg-black/20">
          <div className="flex items-center justify-between mb-2 md:mb-3 px-1">
            <span className="text-[10px] font-bold text-nomad-green uppercase flex items-center gap-1.5 tracking-widest">
              <Newspaper className="w-3 h-3" /> LATEST
            </span>
            <div className="flex items-center gap-1">
              <button
                onClick={prevPost}
                className="p-1 hover:text-nomad-green text-white/40 transition-colors cursor-none"
          ...
          >
            {currentPost.imageUrl ? (
              <div className="aspect-[3] md:aspect-[2] w-full overflow-hidden">
                <img
                  src={formatImageUrl(currentPost.imageUrl)}
                  alt={currentPost.title}
                  className="w-full h-full object-cover opacity-60 group-hover:opacity-100 transition-all duration-500 group-hover:scale-105 mix-blend-luminosity hover:mix-blend-normal"
                />
              </div>
            ) : (
              <div className="aspect-[3] md:aspect-[2] w-full bg-nomad-green/10"></div>
            )}
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent pointer-events-none" />
            <div className="absolute bottom-0 left-0 right-0 p-2 md:p-3">
              <h4 className="text-[11px] font-black uppercase text-white tracking-widest group-hover:text-nomad-green transition-colors line-clamp-2 leading-snug">
                {currentPost.title}
              </h4>
            </div>
          </Link>
      ...
      <div
        className="flex-1 p-4 md:p-8 flex flex-col font-sans font-black text-sm uppercase tracking-wider text-white/50 relative bg-nomad-charcoal/40 backdrop-blur-sm overflow-y-auto min-h-0"
        data-lenis-prevent
      >
        <div className="flex-1 flex flex-col w-full [@media(max-height:800px)_and_(max-width:1023px)]:flex-row [@media(max-height:800px)_and_(max-width:1023px)]:items-center [@media(max-height:800px)_and_(max-width:1023px)]:gap-4">
          <div className="flex flex-col gap-[14px] mt-auto mb-auto pb-8 justify-start pt-4 flex-1 [@media(max-height:800px)_and_(max-width:1023px)]:pb-0 [@media(max-height:800px)_and_(max-width:1023px)]:pt-0">
            {sections.map(({ id, label }) => {
              const isActive = activeSection === id;
              return (
                <button
                  key={id}
                  className={`text-left w-full flex items-center gap-3 cursor-none transition-colors py-1 shrink-0 ${isActive ? "text-nomad-green" : "hover:text-white"}`}
                  onMouseEnter={() => setCursorVariant("hover")}
                  onMouseLeave={() => setCursorVariant("default")}
                  onClick={() => scrollToSection(id)}
                >
                  <div
                    className={`w-2 h-2 rounded-full transition-all flex-shrink-0 ${isActive ? "bg-nomad-green shadow-[0_0_10px_rgba(34,197,94,0.8)]" : "bg-transparent"}`}
                  />
          ...
          <div className="mt-4 flex flex-col gap-3 shrink-0 w-full [@media(max-height:800px)_and_(max-width:1023px)]:w-[130px] [@media(max-height:800px)_and_(max-width:1023px)]:mt-0">
            <button
              onClick={handleJoin}
              onMouseEnter={() => setCursorVariant("waitlist")}
              onMouseLeave={() => setCursorVariant("default")}
              className="w-full bg-[#FFD700] text-[#000] px-4 py-4 font-black text-sm [@media(max-height:800px)_and_(max-width:1023px)]:text-xs uppercase tracking-widest cursor-none hover:bg-nomad-green  hover:text-white transition-colors flex items-center justify-center gap-2 group shadow-[4px_4px_0px_rgba(0,0,0,0.5)]"
            >
            ...
            <div className="flex gap-2">
              <button
                ...
                onMouseEnter={() => setCursorVariant("hover")}
                onMouseLeave={() => setCursorVariant("default")}
                className="flex-1 border border-white/20 text-white/70 px-4 py-3 font-black text-xs uppercase tracking-widest cursor-none hover:bg-white/10 hover:text-white transition-colors flex justify-center items-center group"
              >
                {isSoundEnabled ? (
              ...
              <button
                ...
                onMouseEnter={() => setCursorVariant("hover")}
                onMouseLeave={() => setCursorVariant("default")}
                className="flex-1 border border-nomad-green bg-nomad-green text-[#000] px-4 py-3 font-black text-xs uppercase tracking-widest cursor-none hover:bg-[#22C55E]/80 hover:text-[#000] transition-colors flex justify-center items-center group"
              >
                {theme === "dark" ? (
...
export default function Hero({
          ...
          modalPlay.catch((e) => {
            ...
            if (modalVideoRef.current) {
              ...
              modalVideoRef.current
                .play()
                .catch((err) => console.log("Modal play error muted:", err));
            }
          });
        }
      } else {
  ...
  return (
    <section
      ref={containerRef}
      id="hero-section"
      className="relative min-h-screen flex flex-col overflow-hidden bg-nomad-charcoal font-sans"
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col md:flex-row w-full border-b border-white/10">
        ...
        <div className="flex-1 flex flex-col relative justify-center items-center py-24 md:py-0 w-full overflow-hidden">
          {/* Grid Crosshairs */}
          <div className="absolute top-10 left-10 text-white/20 hidden md:block">
            +
          </div>
          ...
          <div className="absolute bottom-10 right-10 text-white/20 hidden md:block">
            +
          </div>
          ...
          >
            ...
            >
              The ultimate event engagement platform. We help you turn your
              events into experiences. Ready to create, join and participate in
                ...
                >
                  <div className="-rotate-45 flex flex-col items-center justify-center">
                    ...
                    >
                      <ArrowDown
                        strokeWidth={2.5}
                        className="w-5 h-5 md:w-6 md:h-6 text-white group-hover:text-[#111]"
                      />
                    </motion.div>
                  </div>
                </motion.div>
